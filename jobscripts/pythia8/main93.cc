...
int main(int argc, char* argv[]) {
  cout << "PYTHIA started" << endl; //Christal
  ...
  cout << "PYTHIA checked for Hepmc on" << endl; //Christal
  Pythia8ToHepMC ToHepMC;
  if (hepmc){
    cout << "PYTHIA in Hepmc loop" << endl; //Christal
    ToHepMC.setNewFile((out == "" ? "pythia.hepmc" : out + ".hepmc"));
  }
  // Rivet initialization.
  cout << "PYTHIA checked for Rivet on" << endl; //Christal
  ...
  cout << "PYTHIA making log file" << endl; //Christal
  ofstream logBuf;
